...
class FunkinShader extends FlxShader implements IHScriptCustomBehaviour {
	...
	private static function _processGLSLText(source:String, glVersion:String, isFragment:Bool, ?pragmas:Map<String, String>):String {
		...
		if (pragmas != null) {
			...
			source = pragmaKeyword.map(source, (_) -> {
				...
				var pragma = pragmas.exists(name) ? pragmas.get(name) : "";
				if (name == "header") {
					pragma = '\n#ifdef CUSTOM_SIZE\n#define openfl_TextureSize CUSTOM_SIZE\n#endif' + pragma;
					if (canInjectFragColorFix && !injectedFragColor) {
	...
	override function set_glVersion(value:Null<String>):String {
		if (value == null || value == "") value = Shader.getDefaultGLVersion();
	...
	override function set_glFragmentSource(value:String):String {
		if (value == null || value == "") value = ShaderTemplates.defaultFragmentSource;
		if ((__glFragmentSourceRaw = value) != null) {
			if (__glVersion != (__glVersion = Shader.getGLSLTextVersion(value, __glVersion))) {
				__glVersionRaw = __glVersion;
				__glSourceDirty = true;
			}
	...
	override function set_glVertexSource(value:String):String {
		if (value == null || value == "") value = ShaderTemplates.defaultVertexSource;
